...
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        logging.debug(f"Generated links: {links}")
        ...
        logging.info(f"QR code generated and saved to {qr_code_full_path}")
...
async def list_qr_codes_endpoint(token: str = Depends(oauth2_scheme)):
    logging.info("Received request to list QR codes.")
    try:
        logging.debug("Retrieving QR code files from directory.")
        qr_files = list_qr_codes(QR_DIRECTORY)
        logging.debug(f"Files retrieved: {qr_files}")

        responses = []
        for qr_file in qr_files:
            logging.debug(f"Processing file: {qr_file}")
            try:
                qr_code_url = decode_filename_to_url(qr_file[:-4])
                link = generate_links("list", qr_file, SERVER_BASE_URL, f"{SERVER_BASE_URL}/{SERVER_DOWNLOAD_FOLDER}/{qr_file}")
                ...
                logging.error(f"Failed to process file {qr_file}: {e}")

        return responses
    except Exception as e:
        logging.error(f"Failed to list QR codes: {e}")
        raise HTTPException(status_code=500, detail="Internal Server Error")
